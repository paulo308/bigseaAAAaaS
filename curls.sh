...
function test_signup() {
	call "user=teste2&pwd=@bC12345&fname=teste&lname=teste&email=eduardo.morais@gmail.com" "signup_data" "User signed up with success."
	get_token "user=teste&pwd=@bC12345"
	if [ "$token" == "null" ]
	then
		echo 'null token!'
...
function test_favorite() {
	...
	call "username=teste2&item_id=b&item_type=a&city_id=1&country_id=2&favorite_id=b&data=aaa&token=$token" "create_favorite" "Favorite association successfully created."
	call "username=teste2&city_id=1&country_id=2&token=$token" "read_favorite" "Favorite association successfully read." 
	call "username=teste2&token=$token" "read_favorites" "Favorite association successfully read." 
	call "username=teste2&token=errado" "read_favorites" "Invalid token." 
	call "username=errado&token=$token" "read_favorites" "Invalid username." 
	call "username=teste2&item_id=b&token=$token" "delete_favorite" "Favorite association successfully deleted."
